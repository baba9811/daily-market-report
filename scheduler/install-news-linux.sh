...
# ============================================================
# Daily Scheduler - Install Linux cron Job (News Briefing)
# ============================================================
...
echo "=== Daily Scheduler - News Briefing Linux Installer ==="
echo ""
...
# Remove existing daily-scheduler-news cron entry
EXISTING_CRON=$(crontab -l 2>/dev/null || true)
NEW_CRON=$(echo "$EXISTING_CRON" | grep -v "$CRON_MARKER" || true)

# Add new cron entry
CRON_LINE="$MINUTE $HOUR * * * /bin/bash $PROJECT_DIR/scheduler/run_news.sh $CRON_MARKER"
if [ -n "$NEW_CRON" ]; then
    NEW_CRON="$NEW_CRON
$CRON_LINE"
else
...
if crontab -l 2>/dev/null | grep -q "$CRON_MARKER"; then
    ...
    echo ""
    echo "Commands:"
    echo "   Run now:    make news-scheduler-linux-start"
    echo "   Uninstall:  make news-scheduler-linux-uninstall"
    echo "   View logs:  tail -f $PROJECT_DIR/logs/scheduler.log"
else
    echo "Failed to install news briefing scheduler. Check cron service status."
    exit 1
fi
