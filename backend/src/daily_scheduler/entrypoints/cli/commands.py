...
def run_news(
    ...
    """Run the Korean news briefing pipeline."""
    setup_logging(verbose)
    from daily_scheduler.database import (
        get_session_factory,
    )
    ...
    console.print("[bold blue]Starting Korean news briefing pipeline...[/bold blue]")
    session_factory = get_session_factory()
    session = session_factory()
    try:
        pipeline = get_news_pipeline(session)
        success = pipeline.execute()
        session.commit()
    except Exception:
        session.rollback()
    ...
    if success:
        console.print("[bold green]News briefing completed successfully![/bold green]")
    else:
        console.print("[bold red]News briefing failed. Check logs for details.[/bold red]")
        raise typer.Exit(code=1)


@app.command(name="run-global-news")
def run_global_news(
    verbose: bool = typer.Option(
        False,
        "--verbose",
    ...
    """Run the international/global news briefing pipeline."""
    setup_logging(verbose)
    from daily_scheduler.database import (
        get_session_factory,
    )
    from daily_scheduler.infrastructure.dependencies import (
        get_global_news_pipeline,
    )

    console.print("[bold blue]Starting global news briefing pipeline...[/bold blue]")
    session_factory = get_session_factory()
    session = session_factory()
    try:
        pipeline = get_global_news_pipeline(session)
        success = pipeline.execute()
        session.commit()
    except Exception:
        session.rollback()
    ...
    if success:
        console.print("[bold green]Global news briefing completed successfully![/bold green]")
    else:
        console.print("[bold red]Global news briefing failed. Check logs for details.[/bold red]")
        raise typer.Exit(code=1)
